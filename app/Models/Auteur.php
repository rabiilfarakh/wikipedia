...
class Auteur extends User{
    ...
    public function updateWiki($titre, $content, $idUser, $idCategorie, $image, $idWiki) {
        try {
            $query = "UPDATE wikis SET nameWiki = ?, contentWiki = ?, idUser = ?, idCategorie = ?, image = ? WHERE idWiki = ?";
            $stmt = $this->db->prepare($query);
            $stmt->bindParam(1, $titre);
            $stmt->bindParam(2, $content);
            $stmt->bindParam(3, $idUser);
            $stmt->bindParam(4, $idCategorie);
            $stmt->bindParam(5, $image);
            $stmt->bindParam(6, $idWiki);
            $stmt->execute();

            return true;
    
        } catch (PDOException $ex) {
            die("Erreur lors de la mise à jour du wiki : " . $ex->getMessage());
        }
    }
